...
const Admin: FC = () => {
  ...
  const [, setLocalAdmin] = useLocalStorage<TUser | undefined>('user', undefined);
  const [, setSessionAdmin] = useSessionStorage<TUser | undefined>('user', undefined);
      ...
      >
        ...
        <form
          ...
          onSubmit={(e) => {
            e.preventDefault();
            if (rememberMe) {
              if (auth.email === AdminAccount.email && auth.password === AdminAccount.password) {
                setLocalAdmin(AdminAccount);
                setTimeout(() => {
